...
/* ================= guide ================= */
/* Two parts. "תראי לי איך": each task runs a guided tour on the real screen
   (tour.js), so she never has to remember where a button is. "כדאי לדעת":
   the few ideas a button cannot show, one short fold each. */
async function viewGuide() {
  setTitle('מדריך');
  const facts = [
    ['leaf', 'מה האפליקציה עושה', `
      <p>רושמים מה אוכלים, ורואים כמה נשאר עד היעד של היום. היעד מחושב במיוחד בשבילך, ואחרי שלושה שבועות האפליקציה בודקת מה קורה בפועל ומציעה לתקן אותו.</p>
      <p>הכול נשמר רק בטלפון שלך, ומגובה לבד.</p>`],
    ['home', 'העיגול במסך הראשי', `
      <p>העיגול מראה כמה נשאר לאכול היום. ירוק זה מצוין, צהוב זה קרוב ליעד. אחרי ארבע אחר הצהריים כתוב בו כמה נשאר לערב.</p>`],
    ['moon', 'יום שנגמר מעל היעד', `
      <p>זה בסדר גמור. מה שקובע הוא הממוצע של השבוע, ומחר יום חדש.</p>`],
    ['scale', 'המשקל קופץ', `
      <p>זה נורמלי. מים ומלח מזיזים קילו ויותר מיום ליום. לכן האפליקציה מסתכלת על הממוצע של שבוע ועל היקף המותניים.</p>`],
    ['bulb', 'הנקודה הצבעונית ליד מאכל', `
      <p>ירוק זה קל, צהוב זה בינוני, כתום זה עשיר בקלוריות. שום דבר לא אסור, כדאי לשים לב לכמות.</p>`],
    ['gear', 'הטקסט קטן מדי', `
      <p>ב"עוד" ואז "הגדרות ויעד" יש "גודל טקסט". בוחרים גדול או ענק.</p>`],
  ];
  setView(`
    <div class="card soft"><h3>לומדים תוך כדי</h3>
      <p class="note" style="color:var(--text)">בוחרים מה רוצים לעשות, והאפליקציה מראה על המסך איפה ללחוץ, צעד אחרי צעד. עושים את זה באמת, לא רק קוראים.</p>
      <p class="note">בכל מסך יש גם ${icon('help', 'sm')} למעלה, לעזרה במסך הזה.</p></div>
    <h2 class="section">${icon('sparkle')}תראי לי איך</h2>
    <div class="tasks">${Tour.TOURS.map((t) => `<button class="task" data-tour="${t.id}">
      <span class="tic">${icon(t.icon)}</span>
      <span class="grow"><span class="tt">${esc(t.title)}</span><br><span class="ts">${esc(t.sub)}</span></span>
      <span class="go">${icon('chev')}</span></button>`).join('')}</div>
    <h2 class="section">${icon('bulb')}כדאי לדעת</h2>
    <div class="guide">${facts.map(([ic, title, body]) => `<details>
